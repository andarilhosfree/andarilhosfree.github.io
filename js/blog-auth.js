(function () {
	...
	function initBlogPageAuth() {
		var loginNav = document.getElementById('blogAuthLogin');
		var userNav = document.getElementById('blogAuthUser');
		var userLabel = document.getElementById('blogAuthUserLabel');
		var logoutBtn = document.getElementById('blogAuthLogout');
		var newPostNav = document.getElementById('blogAuthNewPost');
		var googleBtn = document.getElementById('blogGoogleSignIn');
		var loginError = document.getElementById('blogLoginError');
		var configNotice = document.getElementById('blogLoginConfigNotice');
		...
		function updateUi(user, member) {
			...
			setVisible(newPostNav, signedIn && canBlog);
			...
			if (signedIn && userLabel) {
				...
				if (canBlog) {
					...
					userLabel.textContent = label + ' (membro)';
				} else {
		...
		googleBtn.addEventListener('click', function () {
			...
			window.AndarilhosAuth.signInWithGoogle()
				.then(function () {
					if (window.jQuery) {
						window.jQuery('#blogLoginModal').modal('hide');
					}
				})
				.catch(function (err) {
	...
	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', initBlogPageAuth);
	} else {
		initBlogPageAuth();
	}
})();
